                ...
                <ul id="friendship-buttons-ul" class="nav navbar-nav navbar-right">
                    <li id="friendship-buttons-li" style="margin-left: 15px;">
                        ...
                        @elseif(Auth::user()->hasFriendRequestReceived($user))
                            <!--Accept-->
                            <form id="accept-friendship" class="inline" action="/friends/accept/{{$user->id}}" method="post">
                            ...
                            <!--Reject-->
                             <form id="reject-friendship" class="inline" action="/friends/reject/{{$user->id}}" method="post">
                                <div class="form-group inline">
                        ...
                        @elseif(Auth::user()->isFriendsWith($user))
                            <form id="delete-friendship" action="/friends/delete/{{$user->id}}" method="post">
                                <button class="btn btn-danger navbar-btn">Arkadaşlıktan Çıkar</button>
                                <input type="hidden" name="_token" value="{{ csrf_token() }}">
                            </form>

                        <!--If users are not Friends-->
                        @elseif(Auth::user()->id!==$user->id)
                            <form id="send-friendship" action="/friends/add/{{$user->id}}" method="post">
                                <button class="btn btn-success navbar-btn">Arkadaşlık İsteği Gönder</button>
                                <input type="hidden" name="_token" value="{{ csrf_token() }}">
                            </form>
                        @endif
                    </li>

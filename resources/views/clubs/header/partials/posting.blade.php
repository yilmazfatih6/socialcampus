...
<form role="form" enctype="multipart/form-data" action="/club/post/{{$club->id}}" method="post">
    <!--Text Area-->
    <div class="form-group{{ $errors->has('post') ? ' has-error' : '' }}">
        <textarea placeholder="{{$club->name}} içinde bir şeyler paylaş..." name="post-{{$club->id}}" class="form-control" rows="2"></textarea>
        @if($errors->has("post-{$club->id}"))
            <span class="help-block">{{$errors->first("post-{$club->id}")}}</span>
        @endif
    </div>
    <!--Sharing Image-->
